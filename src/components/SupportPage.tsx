...
export function SupportPage() {
  ...
  const [returnStatus, setReturnStatus] = useState<'idle' | 'validating' | 'approved'>('idle');

  const validationSteps = [
    { step: 1, label: 'Solicitud recibida', completed: returnStatus !== 'idle' },
    { step: 2, label: 'Revisión de productos', completed: returnStatus === 'approved' || returnStatus === 'validating' },
    { step: 3, label: 'Verificación de condiciones', completed: returnStatus === 'approved' },
    { step: 4, label: 'Aprobación final', completed: returnStatus === 'approved' }
  ];
          ...
          <TabsContent value="tickets" className="space-y-8">
            ...
            <Card className="p-8 rounded-2xl border border-gray-100">
              ...
              <div className="space-y-6">
                {/* Channel Selection */}
                <div>
                  <Label className="mb-3 block">Selecciona el canal de contacto</Label>
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    >
                      <MessageSquare className={`w-8 h-8 mx-auto mb-3 ${
                        selectedChannel === 'chat' ? 'text-blue-600' : 'text-gray-600'
                      }`} />
                      <p>Chat en Vivo</p>
                      <p className="text-xs text-gray-600 mt-1">Respuesta inmediata</p>
                    </button>
                    ...
                    >
                      ...
                      <p>Correo</p>
                      <p className="text-xs text-gray-600 mt-1">Respuesta en 24h</p>
                    </button>
                    ...
                    >
                      <AlertCircle className={`w-8 h-8 mx-auto mb-3 ${
                        selectedChannel === 'complaint' ? 'text-red-600' : 'text-gray-600'
                      }`} />
                      <p>Reclamo</p>
            ...
            <div>
              ...
              <div className="space-y-4">
                {mockTickets.map((ticket) => (
                  <Card key={ticket.id} className="p-6 rounded-2xl border border-gray-100 hover:shadow-lg transition-all duration-300">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">
                          Ticket #{ticket.id} • Canal: {ticket.channel === 'chat' ? 'Chat' : ticket.channel === 'email' ? 'Correo' : 'Reclamo'}
                        </p>
                      </div>
                      ...
                      {ticket.messages.map((msg) => (
                        ...
                        >
                          ...
                          <p className="text-xs text-gray-500 mt-2">
                            {msg.sender === 'customer' ? 'Tú' : 'Soporte'} • {msg.timestamp.toLocaleTimeString('es-EC', { hour: '2-digit', minute: '2-digit' })}
                          </p>
                        </div>
                      ))}
                    </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-blue-800 mb-4">
                    Puedes devolver cualquier producto dentro de los 7 días posteriores a la entrega, 
                    siempre que esté en perfectas condiciones y con las etiquetas originales.
                  </p>
                  ...
                  <div className="space-y-3">
                    {mockOrder.items.map((item, index) => (
                      <div key={index} className="flex items-center gap-4">
                        ...
                        <p className="text-pink-600">${(item.product.price * item.quantity).toFixed(2)}</p>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div className="space-y-3">
                      {validationSteps.map((step) => (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <p className={`text-sm ${step.completed ? 'text-green-900' : 'text-gray-700'}`}>
                  ...
                  <div className="space-y-4">
                    <div className="p-6 bg-green-50 border border-green-200 rounded-xl">
                      ...
                      <p className="text-green-800 text-sm">
                        Tu solicitud de devolución ha sido aprobada. Procede con el reembolso para completar el proceso.
                      </p>
                    </div>
